...
export default function Elections() {
    ...
    }, [isReady, isConnected, fetchElections])
        ...
        {loading || !isReady ? (
            ...
            ) : <div className="flex flex-col sm:flex-row flex-wrap justify-between items-center border rounded-lg p-10 gap-14 mt-10 form-item">
            {
                elections.map((election, index) => (
                    <div key={index} className="shadow-md rounded-lg flex flex-col justify-center items-center py-12 px-16 text-center basis-[45%] form-item">
                        <p className="text-[34px] font-medium form-item">{election[0]}</p>
                        <p className="text-md font-normal text-[#8F96A1] form-item">Election running from - {new Date(Number(election[1]) * 1000).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })} to {new Date(Number(election[2]) * 1000).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                        
                        <div className="flex items-center justify-center gap-6 flex-wrap mt-10">
                            <button className="space-x-4 bg-[#5773fb] text-white rounded-lg w-[9rem] h-[3rem] mt-6 form-item transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300" onClick={(e) => handleRoute(e, index)}>
                                Click To View
                            </button>
                            <button className="space-x-4 bg-[#5773fb] text-white rounded-lg w-[9rem] h-[3rem] mt-6 form-item transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300" onClick={(e) => handleAddCandidate(e, index)}>
                                Add Candidate
                            </button>
                            <button className="space-x-4 bg-[#5773fb] text-white rounded-lg w-[9rem] h-[3rem] mt-6 form-item transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300" onClick={(e) => handleAccredit(e, index)}>
                                Accredit Voter
                            </button>
                        </div>
                    </div>
